      ...
      <div v-if="show" class="row">
        <div class="col">
          ...
          <div class="card">
            <div class="card-header text-primary bg-transparent font-weight-bold text-left">
              ...
              />
              <b-form-checkbox class="float-right" v-model="escuela" switch
        ...
        >
          <div class="form-group label-floating mt-3">
            <label class="font-weight-bold" for="nroPresu">Pedido Nro</label>
            <input
              type="Text"
              v-model="buscaPresu"
              id="nroPresu"
        ...
        <div class="col col-xl mt-1">
          <div v-if="detalleVenta.length > 0 " class="form-group col">
            <button class="btn btn-sm btn-primary" @click="savePedido()">Guardar Pedido</button>

            <button class="btn btn-sm btn-danger" @click="cancelarPedido()">Cancelar Pedido</button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    buscarPedido() {
      this.detalleVenta = [];
      axios
        .post("api/searchPedido", { presupuesto: this.buscaPresu })
        .then(res => {
          if (res.data != 20) {
            let datosPresu;
    ...
    savePedido(autorizar) {
      axios.post('api/addPedido',{cliente:this.idCliente,total:this.total,direccion:this.direccion,pedido:this.detalleVenta,escuela:this.escuela}).then(res =>{
        if(res.data==0){
          alertify.success("Pedido Cargado Correctamente");
          this.limpia();
        }
    ...
    cancelarPedido() {
      ...
      alertify.error("Pedido cancelado correctamente");
    },
